...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or gcoin.conf setting gen)\n"
...
Value setgenerate(const Array& params, bool fHelp)
{
    ...
    GenerateGcoins(fGenerate, pwalletMain, nGenProcLimit);
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    pblocktemplate = CreateNewBlock(scriptPubKeyIn, pwalletMain, true);
    if (!pblocktemplate)
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx) {
        ...
        // Because coinbase transactions are signed, send them to mining pool.
        /*
        if (tx.IsCoinBase())
            continue;
        */
    ...
    result.push_back(Pair("coinbasevalue", ValueFromAmount(pblock->vtx[0].vout[0].mValue)));
    ...
    result.push_back(Pair("starttime", pblock->GetBlockStartTime()));
...
Value submitblock(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw std::runtime_error(
            _(__func__) + " \"hexdata\" ( \"jsonparametersobject\" )\n"
